...
class UserService extends \Nette\Object
{
	...
	/**
	 * Nastaví ztracené heslo a vygeneruje nové
	...
	public function user_setLostPassword($userID)
	{
		...
		$lostPasswordData = array(
			...
			'token' => $newToken,
			'valid' => 1,
			'expire' => new \Nette\Utils\DateTime('NOW ' . $this->config['user']['passwordRevokeExpiration']),
			'created' => new \Nette\Utils\DateTime(),
		);

		$this->user_expireAllLostPasswords($userID);
