...
class RegistrationControl extends \Nette\Application\UI\Control
{
	...
	public function render()
	{
		...
		{
			$templateName = \zvitek\Helper::control_template(__DIR__,'Activation');
			...
			$templateName = \zvitek\Helper::control_template(__DIR__,'Registration');
	...
	public function registrationForm_submitted(\Nette\Application\UI\Form $form)
	{
		...
		if($newUser)
		{
			...
			if($this['mailSend']->sendActivationEmail())
			{
				...
				$this->getPresenter()->redirect($this->config['user']['redirectAfterRegistration']);
			}
			else
			{
